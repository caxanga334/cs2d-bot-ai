...
function fai_fightnpc(id)
	if object(vai_targetnpc[id],"exists") then
		if object(vai_targetnpc[id],"health")>0 then
			-- Melee Combat?
			if itemtype(player(id,"weapontype"),"range")<50 then
				-- Yes, melee! Run to target
				if ai_goto(id,object(vai_targetnpc[id],"tilex"),object(vai_targetnpc[id],"tiley"))~=2 then
					vai_mode[id]=0
				end
			else
