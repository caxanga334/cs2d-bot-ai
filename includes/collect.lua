...
function fai_collect(id)
	...
	if vai_itemscan[id]>100 then
		...
		if vai_mode[id]~=6 and vai_mode[id]~=11 and vai_mode[id]~=12 and vai_mode[id]~=63 and not(vai_mode[id]>=20 and vai_mode[id]<=24) and not(vai_mode[id]>=30 and vai_mode[id]<=32) and not(player(id,"team")==1 and vai_set_gm==4) then
